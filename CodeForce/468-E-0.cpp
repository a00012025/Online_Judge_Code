...
bool mark[maxn] ;
LL dp[maxn][maxk][2],dp2[maxk][2],tmp[maxk][2] ;
void DP(int x)
{
    ...
    {
        int xi=v2[x][i].to , w=v2[x][i].w ;
        for(int j=0;j<maxk;j++) for(int k=0;k<2;k++)
            tmp[j][k]=dp2[j][k] , dp2[j][k]=0 ;
        for(int j=0;j<maxk;j++) for(int j2=0;j+j2<maxk;j2++)
        {
            add(dp2[j+j2][0],tmp[j][0]*(dp[xi][j2][0]+dp[xi][j2][1])) ;
            add(dp2[j+j2][1],tmp[j][1]*(dp[xi][j2][0]+dp[xi][j2][1])) ;
            if(mark[x]||mark[xi]) continue ;
            add(dp2[j+j2+1][1],(tmp[j][0]*dp[xi][j2][0]%MOD)*w) ;
        }
    }
    for(int i=0;i<maxk;i++) for(int j=0;j<2;j++)
        dp[x][i][j]=dp2[i][j] ;
}

struct E{int x,y,w;}; /// edge ;
vector<E> edge ;
LL *solve1()
{
    edge.clear() ;
