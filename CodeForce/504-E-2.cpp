...
#define DB double
using namespace std;
const int maxn=300000+10,maxq=1000000+10,maxst=37000000 ;
const LL X=123LL ;
...
int Scnt=0 ;
node *v2[maxn] ;
void add_node(int x,int _v,int _i)
{
    node *u=POOL+Scnt ;
    if(Scnt==maxst){printf("FUCK\n") ; exit(1) ;}
    u->val=_v ; u->id=_i ;
    u->nex=v2[x] ; v2[x]=u ;
    Scnt++ ;
}
...
{
    vis[x]=1 ;
    for(node *i=v2[x];i;i=v2[x])
    {
        if(vis[i->val]) ansval[i->id]=getbs(i->val) ;
        v2[x]=i->nex ;
    }
    for(auto i : v[x]) if(i!=f)
...
struct query
{
    ...
    void get(int qid)
    {
        x1=getint() , y1=getint() ;
        x2=getint() , y2=getint() ;
        add_node(x1,y1,2*qid-1) ;
        add_node(y1,x1,2*qid-1) ;
        add_node(x2,y2,2*qid) ;
        add_node(y2,x2,2*qid) ;
    }
    void get2(int qid)
    {
...
bool process_query()
{
    ...
    for(int i=1;i<=Qnum;i++) if(q[i].l+1!=q[i].r)
    {
        ...
        if(dep[x1]-dep[lca1]+1>=len)
            add_node(x1,len,2*i-1) ;
        else
            add_node(y1,dep[x1]+dep[y1]-2*dep[lca1]+1-len,2*i-1) ;
        if(dep[x2]-dep[lca2]+1>=len)
            add_node(x2,len,2*i) ;
        else
            add_node(y2,dep[x2]+dep[y2]-2*dep[lca2]+1-len,2*i) ;
    }
    if(ok) return 1 ;
    dfs_solve_fa(0,0) ;
    for(int i=1;i<=Qnum;i++) if(q[i].l+1!=q[i].r)
