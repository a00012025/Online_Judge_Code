...
main()
{
    ...
    for(int i=1;i<=n;i++) dis[i]= r[i]==n+1 ? 0 : L[r[i]]-rmax[i] ;
    for(int i=1;i<=n;i++) debugf("%d,",dis[i]) ; debugf("\n") ;

    for(int i=0;i<20;i++) for(int j=1;j<=n;j++) newr[i][j]=n+1 ;
    for(int i=1;i<=n;i++)
        cost[0][i]=dis[i] , newr[0][i]=r[i] , debugf("newr[%d][%d]=%d\n",0,i,r[i]) ;
    for(int i=1;i<20;i++) for(int j=1;j<=n;j++) if(newr[i-1][j]!=n+1)
    {
        newr[i][j]= newr[i-1][newr[i-1][j]] ;
        cost[i][j]= cost[i-1][j]+cost[i-1][newr[i-1][j]] ;
        debugf("newr[%d][%d]=%d\n",i,j,newr[i][j]) ;
        debugf("cost[%d][%d]=%d\n",i,j,cost[i][j]) ;
    }
    ...
    while(Q--)
    {
        ...
        {
            int i ;
            for(i=19;i>=0 && newr[i][x]>y;i--) ;
            //if(i==-1) { ans+=cost[0][x] ; break ; }
            debugf("i=%d , x=%d\n",i,x) ;
            ans+=cost[i][x] ; x=newr[i][x] ;
        }
        printf("%d\n",ans) ;
    }
}
