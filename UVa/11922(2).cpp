...
struct SplayTree
{
    ...
    void splay(node *&u,int k)
    {
        ...
        if(d1!=-1)
        {
            ...
            pushdown(p) ;
            int d2=cmp(p,k) ;
            int k2= (d2==0 ? k : k-lsize(p)-1 ) ;
            if(d2!=-1)
            {
                if(d2==0) splay(p->l,k2) ;
                else splay(p->r,k2) ;
                if(d1==0 && d2==0) right_rotate(u) ;
                if(d1==0 && d2==1) left_rotate(u->l) ;
                if(d1==1 && d2==0) right_rotate(u->r) ;
                if(d1==1 && d2==1) left_rotate(u) ;
            }
