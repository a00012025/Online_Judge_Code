...
void delete_contained()
{
    ...
    {
        if(contain(a[j],a[i])) ok[i]=0 ;
        if(contain(rev[j],a[i])) ok[i]=0 ;
    }
    int cnt=0 ;
    for(int i=0;i<n;i++) if(ok[i])
    {
        if(cnt==i) {cnt++ ; continue ;}
        a[cnt]=a[i] ; rev[cnt]=a[i] ;cnt++ ;
    }
    n=cnt ;
}
...
void find_paste_num()
{
    ...
    {
        pas[i][j]=paste(a[i],a[j]) ;
        pas[i][j+n]=paste(a[i],rev[j]) ;
        pas[i+n][j]=paste(rev[i],a[j]) ;
        pas[i+n][j+n]=paste(rev[i],rev[j]) ;
        //printf("pas[%d][%d]=%d\n",i,j,pas[i][j]) ;
    }
    //cout << a[0].s << endl ;
    //printf("%d\n",pas[0][0]) ;
...
main()
{
    while(scanf("%d",&n)==1 && n)
    {
        ...
        delete_contained() ;
        find_paste_num() ;
        for(int i=0;i<n;i++) len[i]=len[i+n]=a[i].size() ;
