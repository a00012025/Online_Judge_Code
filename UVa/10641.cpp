...
bool segment_proper_inter(pt a,pt b,pt c,pt d)
{
...
bool check(int i,int j) /// q[i] can see p[j]~p[(j+1)%n] ?
{
    ...
    for(int k=0;k<n;k++)
    {
        if(k!=j && on_segment(q[i],p[j],p[k])) return 0 ;
        if(on_segment(q[i],mid,p[k])) return 0 ;
        if(segment_proper_inter(q[i],p[j],p[k],p[(k+1)%n])) return 0 ;
        if(segment_proper_inter(q[i],p[(j+1)%n],p[k],p[(k+1)%n])) return 0 ;
        if(segment_proper_inter(q[i],mid,p[k],p[(k+1)%n])) return 0 ;
    }
    debugf("q[%d] can see p[%d]~p[%d]\n",i,j,(j+1)%n) ;
    return 1 ;
}
...
bool contain(int l,int r,int x,int y) /// l~r contain x~y ?
{
    ...
    if(x>y) y+=n ;
    if(x>=l && y<=r) return 1 ;
    if((x+n)>=l && (y+n)<=r) return 1 ;
    if(x>=(l+n) && y<=(r+n)) return 1 ;
    return 0 ;
}
